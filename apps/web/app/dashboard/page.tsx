  ...
  return (
    <main className="app-shell">
      ...
      <section className="content">
        ...
        <section className="kpi-grid">{[['Monthly revenue', `${currency} ${formatNumber(monthlyRevenue)}`, overview ? 'LIVE' : '—', overview ? 'current month' : 'awaiting tenant data'], ['Active subscribers', formatNumber(activeCustomers), overview ? 'LIVE' : '—', 'active customers'], ['Online sessions', formatNumber(onlineSessions), overview ? 'LIVE' : '—', 'currently connected'], ['Network availability', availabilityLabel, overview ? 'LIVE' : '—', overview && availability == null ? 'no routers registered' : 'current fleet']].map(([label, value, delta, note]) => <article className="kpi" key={label}><div className="kpi-label"><span>{label}</span><button type="button" aria-label={`${label} options`}>•••</button></div><strong>{value}</strong><div className="kpi-foot"><span>{delta}</span><small>{note}</small></div></article>)}</section>
        <section className="hero-grid"><article className="panel revenue-panel"><div className="panel-head"><div><div className="panel-kicker">FINANCIAL PERFORMANCE</div><h2>Revenue performance</h2><p>Successful payments recorded during the current month</p></div></div><div className="revenue-total"><strong>{currency} {formatNumber(monthlyRevenue)}</strong><span>{overview ? 'Live' : '—'}</span><small>{overview ? 'current month successful payments' : 'Sign in to load live revenue'}</small></div><div className="chart"><div className="chart-scale"><span>50M</span><span>35M</span><span>20M</span><span>5M</span><span>0</span></div><div className="chart-body"><div className="grid-line g1"/><div className="grid-line g2"/><div className="grid-line g3"/><div className="grid-line g4"/><div className="bars">{revenueBars.map((h, i) => <span style={{ height: `${h}%` }} key={i}/>)}</div><div className="x-labels"><span>01</span><span>05</span><span>10</span><span>15</span><span>20</span><span>25</span><span>30</span></div></div></div></div></article><article className="panel health-panel"><div className="panel-kicker">NETWORK HEALTH</div><h2>Infrastructure status</h2><p>Across {network.totalRouters} registered routers</p><div className="health-ring"><div><strong>{availabilityLabel}</strong><span>availability</span></div></div><div className="health-stats"><div><span className="health-dot online"/> <strong>{network.online}</strong><small>Operational</small></div><div><span className="health-dot warn"/><strong>{network.degraded}</strong><small>Attention</small></div><div><span className="health-dot down"/><strong>{network.offline}</strong><small>Offline</small></div></div><a className="full-button" href="/network">Open network operations <span>→</span></a></article></section>
        <section className="content-grid"><article className="panel locations-panel"><div className="panel-head"><div><div className="panel-kicker">GLOBAL FOOTPRINT</div><h2>Regional performance</h2><p>Subscriber distribution by location</p></div></div><div className="location-table"><div className="location-header"><span>REGION</span><span>SITES</span><span>SUBSCRIBERS</span><span>REVENUE</span><span>UPTIME</span></div>{(overview?.locations ?? []).map((row, i) => { const name = row.name ?? 'Unknown'; const routers = row.routers ?? 0; const uptime = routers ? Math.round(((row.onlineRouters ?? 0) / routers) * 10000) / 100 : null; return <div className="location-row" key={`${name}-${i}`}><div className="region"><span className="region-code">{name.slice(0,2).toUpperCase()}</span><strong>{name}</strong></div><span>{routers}</span><span>{formatNumber(row.activeUsers ?? 0)}</span><strong>{typeof row.revenue === 'number' ? `${currency} ${formatNumber(row.revenue)}` : '—'}</strong><span className="uptime">● {uptime == null ? 'N/A' : `${uptime}%`}</span></div>; })}</div></article><article className="panel attention-panel"><div className="panel-head"><div><div className="panel-kicker">OPERATIONS</div><h2>Attention required</h2><p>Prioritized events from your network</p></div><span className="alert-count">{overview?.kpis?.paymentFailures ?? 0}</span></div><div className="attention-item"><span className="severity critical"/><div><strong>Payment failures detected</strong><small>{overview?.kpis?.paymentFailures ?? 0} failed attempts · current period</small></div><span>›</span></div>{network.degraded > 0 && <div className="attention-item"><span className="severity warning"/><div><strong>Router requires review</strong><small>{network.degraded} routers · degraded state</small></div><span>›</span></div>}{overview && <div className="attention-item"><span className="severity info"/><div><strong>Voucher inventory</strong><small>Review active voucher batches and expiry status</small></div><span>›</span></div>}<a className="full-button" href="/audit">Review audit events <span>→</span></a></article></section>
