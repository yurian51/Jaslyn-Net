  ...
  return (
    <main className="app-shell dashboard-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-status"><span className="pulse" /><div><strong>{liveDataLabel}</strong><small>{overview ? 'Tenant overview loaded' : loadError ?? 'Dashboard available for inspection'}</small></div></div>
      ...
      <section className="content">
        <header className="topbar dashboard-topbar">
          <div className="topbar-copy"><div className="eyebrow">JASLYN NET / OPERATIONS</div><h1>Business overview</h1><p className="context">Customers, plans, purchases, sessions and network operations in one console.</p></div>
        ...
        <section className="hero-grid">
          <article className="panel revenue-panel">
            <div className="panel-head"><div><div className="panel-kicker">COMMERCIAL PERFORMANCE</div><h2>Revenue performance</h2><p>Successful payment value returned by the tenant overview.</p></div><span className="panel-badge">{overview ? 'LIVE' : 'NO LIVE DATA'}</span></div>
            ...
            <div className="chart" aria-label="Revenue performance chart">
              ...
              <div className="chart-body"><div className="grid-line g1" /><div className="grid-line g2" /><div className="grid-line g3" /><div className="grid-line g4" />{revenueBars.length ? <div className="bars">{revenueBars.map((height, index) => <span style={{ height: `${height}%` }} key={index} aria-label={`Revenue period ${index + 1}`} />)}</div> : <div className="chart-empty"><strong>No series available</strong><span>Connect an authenticated tenant session to render real revenue history.</span></div>}</div>
            </div>
          </article>

          <article className="panel health-panel">
            <div className="panel-kicker">NETWORK HEALTH</div><h2>Infrastructure status</h2><p>{network.totalRouters} registered router{network.totalRouters === 1 ? '' : 's'} in this tenant.</p>
        ...
        <section className="content-grid dashboard-lower-grid">
          <article className="panel locations-panel">
            ...
            {overview?.locations?.length ? <div className="location-table"><div className="location-header"><span>LOCATION</span><span>ROUTERS</span><span>SUBSCRIBERS</span><span>REVENUE</span><span>UPTIME</span></div>{overview.locations.map((row, index) => { const name = row.name ?? 'Unknown'; const routers = row.routers ?? 0; const uptime = routers ? Math.round(((row.onlineRouters ?? 0) / routers) * 10000) / 100 : null; return <div className="location-row" key={`${name}-${index}`}><div className="region"><span className="region-code">{name.slice(0, 2).toUpperCase()}</span><strong>{name}</strong></div><span>{routers}</span><span>{formatNumber(row.activeUsers ?? 0)}</span><strong>{typeof row.revenue === 'number' ? formatMoney(currency, row.revenue) : '—'}</strong><span className="uptime">● {uptime == null ? 'N/A' : `${uptime}%`}</span></div>; })}</div> : <div className="module-empty"><strong>No location data returned</strong><span>{overview ? 'The tenant has no location records in the overview response.' : 'Live regional data is intentionally not fabricated.'}</span><a href="/network">Inspect network inventory →</a></div>}
          </article>
        ...
        <section className="panel operations-panel"><div className="panel-head"><div><div className="panel-kicker">ACTUAL CONSOLE MODULES</div><h2>Operate the business</h2><p>These links map directly to the implemented Jaslyn Net routes, not a wish list disguised as a dashboard.</p></div></div><div className="operations-grid">{operationalAreas.map((area) => <a className="operation-card" href={area.href} key={area.href}><span className="operation-icon">{area.icon}</span><div><strong>{area.title}</strong><small>{area.description}</small></div><span className="operation-arrow">→</span></a>)}</div></section>

        <section className="panel sessions-panel dashboard-sessions"><div className="panel-head"><div><div className="panel-kicker">LIVE NETWORK</div><h2>Active sessions</h2><p>{formatNumber(onlineSessions)} users currently connected.</p></div><div className="session-actions"><span className="live-pill"><i /> LIVE</span><a className="outline-button" href="/sessions">View sessions →</a></div></div><div className="table-wrap"><table><thead><tr>{['Customer', 'Location', 'Gateway', 'Plan', 'IP address', 'Duration', 'Status'].map((heading) => <th key={heading}>{heading}</th>)}</tr></thead><tbody>{liveSessions.map((row, index) => <tr key={`${row.customer ?? 'unknown'}-${row.ip_address ?? 'unknown'}-${index}`}><td>{row.customer ?? '—'}</td><td>{row.location ?? '—'}</td><td>{row.router ?? '—'}</td><td>{row.plan ?? '—'}</td><td>{row.ip_address ?? '—'}</td><td>{row.duration ?? '—'}</td><td><span className={`status ${(row.status ?? 'UNKNOWN').toLowerCase()}`}><i /> {row.status ?? 'UNKNOWN'}</span></td></tr>)}</tbody></table>{!liveSessions.length && <div className="empty-state">{overview ? 'No active sessions were returned for this tenant.' : 'Live session rows are withheld until authenticated tenant data is available.'}</div>}</div></section>
