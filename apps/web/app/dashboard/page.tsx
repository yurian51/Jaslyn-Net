...
type Overview = {
  ...
  sessions?: Array<{ customer?: string; location?: string; router?: string; ip_address?: string; status?: string }>;
...
export default function DashboardPage() {
  ...
  const liveSessions = overview?.sessions?.length ? overview.sessions.map(row => [
    row.customer ?? '—', row.location ?? '—', row.router ?? '—', 'Connected', row.ip_address ?? '—', 'LIVE', row.status ?? 'UNKNOWN',
  ]) : [];
  ...
  return (
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <nav className="nav"><p className="nav-section">OPERATIONS</p>{nav.slice(0, 7).map(([item, icon]) => <a key={item} href={item === 'Overview' ? '/dashboard' : item === 'Customers' ? '/customers' : item === 'Plans & Products' ? '/packages' : item === 'Purchases' ? '/purchases' : `/${item.toLowerCase().replaceAll(' ', '-')}`} className={active === item ? 'nav-item active' : 'nav-item'} onClick={() => setActive(item)}><span className="nav-icon">{icon}</span><span>{item}</span></a>)}<p className="nav-section second">INSIGHTS & CONTROL</p>{nav.slice(7).map(([item, icon]) => <a key={item} href={`/${item.toLowerCase().replaceAll(' ', '-')}`} className={active === item ? 'nav-item active' : 'nav-item'} onClick={() => setActive(item)}><span className="nav-icon">{icon}</span><span>{item}</span></a>)}</nav>
      ...
      <section className="content">
        <header className="topbar"><div><div className="eyebrow">JASLYN NET / {active.toUpperCase()}</div><h1>Operations overview</h1><p className="context">A real-time view of revenue, subscribers, network health and activity.</p></div><div className="top-actions"><button className="date-button">◷ <span>Current period</span></button><button className="selector">All regions <span>⌄</span></button><button className="icon-button" aria-label="Search">⌕</button><button className="icon-button notification" aria-label="Notifications">♧<i/></button><div className="avatar small-avatar">J</div></div></header>
        <section className="kpi-grid">{[['Monthly recurring revenue', `${currency} ${formatNumber(monthlyRevenue)}`, overview ? 'LIVE' : '—', overview ? 'current month' : 'awaiting tenant data'], ['Active subscribers', formatNumber(activeCustomers), overview ? 'LIVE' : '—', 'active customers'], ['Online sessions', formatNumber(onlineSessions), overview ? 'LIVE' : '—', 'currently connected'], ['Network availability', `${availability}%`, overview ? 'LIVE' : '—', 'current fleet']].map(([label, value, delta, note]) => <article className="kpi" key={label}><div className="kpi-label"><span>{label}</span><button aria-label={`${label} options`}>•••</button></div><strong>{value}</strong><div className="kpi-foot"><span>{delta}</span><small>{note}</small></div></article>)}</section>
        <section className="hero-grid"><article className="panel revenue-panel"><div className="panel-head"><div><div className="panel-kicker">FINANCIAL PERFORMANCE</div><h2>Revenue performance</h2><p>Consolidated across all operating regions</p></div><div className="periods">{['7D','30D','90D','1Y'].map(x => <button key={x} onClick={() => setPeriod(x)} className={period === x ? 'selected' : ''}>{x}</button>)}</div></div><div className="revenue-total"><strong>{currency} {formatNumber(monthlyRevenue)}</strong><span>{overview ? 'Live' : '—'}</span><small>{overview ? 'current month' : 'Sign in to load live revenue'}</small></div><div className="chart"><div className="chart-scale"><span>50M</span><span>35M</span><span>20M</span><span>5M</span><span>0</span></div><div className="chart-body"><div className="grid-line g1"/><div className="grid-line g2"/><div className="grid-line g3"/><div className="grid-line g4"/><div className="bars">{revenueBars.map((h, i) => <span style={{ height: `${h}%` }} key={i}/>)}</div><div className="x-labels"><span>01</span><span>05</span><span>10</span><span>15</span><span>20</span><span>25</span><span>30</span></div></div></div></article><article className="panel health-panel"><div className="panel-kicker">NETWORK HEALTH</div><h2>Infrastructure status</h2><p>Across {network.totalRouters} registered routers</p><div className="health-ring"><div><strong>{availability}%</strong><span>availability</span></div></div><div className="health-stats"><div><span className="health-dot online"/> <strong>{network.online}</strong><small>Operational</small></div><div><span className="health-dot warn"/><strong>{network.degraded}</strong><small>Attention</small></div><div><span className="health-dot down"/><strong>{network.offline}</strong><small>Offline</small></div></div><a className="full-button" href="/network">Open network operations <span>→</span></a></article></section>
        <section className="content-grid"><article className="panel locations-panel"><div className="panel-head"><div><div className="panel-kicker">GLOBAL FOOTPRINT</div><h2>Regional performance</h2><p>Subscriber and revenue distribution</p></div><a className="outline-button" href="/analytics">View all regions →</a></div><div className="location-table"><div className="location-header"><span>REGION</span><span>SITES</span><span>SUBSCRIBERS</span><span>REVENUE</span><span>UPTIME</span></div>{(overview?.locations ?? []).map((row, i) => { const name = row.name ?? 'Unknown'; const routers = row.routers ?? 0; const uptime = routers ? Math.round(((row.onlineRouters ?? 0) / routers) * 10000) / 100 : 0; return <div className="location-row" key={`${name}-${i}`}><div className="region"><span className="region-code">{name.slice(0,2).toUpperCase()}</span><strong>{name}</strong></div><span>{routers}</span><span>{formatNumber(row.activeUsers ?? 0)}</span><strong>{currency} 0</strong><span className="uptime">● {uptime}%</span></div>; })}</div></article><article className="panel attention-panel"><div className="panel-head"><div><div className="panel-kicker">OPERATIONS</div><h2>Attention required</h2><p>Prioritized events from your network</p></div><span className="alert-count">{overview?.kpis?.paymentFailures ?? 0}</span></div><div className="attention-item"><span className="severity critical"/><div><strong>Payment failures detected</strong><small>{overview?.kpis?.paymentFailures ?? 0} failed attempts · current period</small></div><span>›</span></div>{network.degraded > 0 && <div className="attention-item"><span className="severity warning"/><div><strong>Router requires review</strong><small>{network.degraded} routers · degraded state</small></div><span>›</span></div>}{overview && <div className="attention-item"><span className="severity info"/><div><strong>Voucher inventory</strong><small>Review active voucher batches and expiry status</small></div><span>›</span></div>}<a className="full-button" href="/security">Review all alerts <span>→</span></a></article></section>
        <section className="panel sessions-panel"><div className="panel-head"><div><div className="panel-kicker">LIVE NETWORK</div><h2>Active sessions</h2><p>{formatNumber(onlineSessions)} users currently connected</p></div><div className="session-actions"><span className="live-pill"><i/> LIVE</span><a className="outline-button" href="/sessions">View sessions →</a></div></div><div className="table-wrap"><table><thead><tr>{['Customer','Location','Gateway','Plan','IP address','Duration','Status'].map(h => <th key={h}>{h}</th>)}</tr></thead><tbody>{liveSessions.map((row, index) => <tr key={`${row[0]}-${row[4]}-${index}`}>{row.map((cell, i) => <td key={i}>{i === 6 ? <span className={`status ${cell.toLowerCase()}`}><i/> {cell}</span> : cell}</td>)}</tr>)}</tbody></table>{!liveSessions.length && <div className="empty-state">{overview ? 'No active sessions were returned for this tenant.' : 'Sign in to load live sessions.'}</div>}</div></section>
        <footer className="footer"><span>JASLYN NET · Network Operations Platform</span><span>{overview ? 'Live tenant data · loaded from API' : 'Authentication required for live tenant data'}</span></footer>
      </section>
    </main>
  );
}
