...
type LoginResponse = { accessToken: string; expiresIn: number; user: { fullName: string; email: string; role: string }; tenant: { name: string; timezone: string; currency: string } };
...
export default function LoginPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const result = await apiFetch<LoginResponse>('/auth/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: email.trim(), password }) });
      storeAccessToken(result.accessToken); router.replace('/dashboard');
    } catch (cause) {
