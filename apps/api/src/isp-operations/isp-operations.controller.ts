...
import { ChangeAccessStateDto, ChangeCustomerServiceStateDto, CreateAccessBindingDto, CreateFiberLineDto, CreateNetworkJobDto, CreateNetworkSiteDto, FiberServiceStatus, ListQueryDto, UpdateNetworkJobDto } from './isp-operations.dto';
...
export class IspOperationsController {
  constructor(private readonly isp: IspOperationsService, private readonly fibers: FiberLinesService) {}
  ...
  @Get('customers/:customerId/service-state') getCustomerServiceState(@Req() req:AuthenticatedRequest,@Param('customerId') customerId:string){return this.isp.getCustomerServiceState(req.user!.tenantId,customerId);}
  @Put('customers/:customerId/service-state') @Roles('OWNER','ADMIN','AGENT') setCustomerServiceState(@Req() req:AuthenticatedRequest,@Param('customerId') customerId:string,@Body() dto:ChangeCustomerServiceStateDto){return this.isp.setCustomerServiceState(req.user!.tenantId,customerId,dto);}
  ...
  @Put('fiber-lines/:id/status') @Roles('OWNER','ADMIN','AGENT') setFiberStatus(@Req() req:AuthenticatedRequest,@Param('id') id:string,@Body() body:{status:FiberServiceStatus}){return this.fibers.setStatus(req.user!.tenantId,id,body.status);}
}
