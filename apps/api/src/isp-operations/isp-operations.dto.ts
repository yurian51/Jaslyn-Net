...
import { IsEnum, IsInt, IsLatitude, IsLongitude, IsOptional, IsString, IsUUID, Max, MaxLength, Min, MinLength } from 'class-validator';

export enum ServiceState {
  PENDING = 'PENDING', ACTIVE = 'ACTIVE', GRACE = 'GRACE', SUSPENDED = 'SUSPENDED', BLOCKED = 'BLOCKED', DISCONNECTED = 'DISCONNECTED', FAULT = 'FAULT',
}
...
export class CreateAccessBindingDto {
  ...
  @IsEnum(AccessType) accessType!: AccessType;
  @IsOptional() @IsString() @MaxLength(160) externalReference?: string;
  @IsOptional() expiresAt?: string;
}

export class ChangeAccessStateDto {
  @IsEnum(AccessState) state!: AccessState;
...
export class ChangeCustomerServiceStateDto {
  @IsEnum(ServiceState) state!: ServiceState;
  @IsString() @MinLength(2) @MaxLength(500) reason!: string;
  @IsOptional() @IsString() @MaxLength(80) source?: string;
  @IsOptional() effectiveAt?: string;
  @IsOptional() expiresAt?: string;
}
...
export class CreateNetworkSiteDto {
  ...
  @IsOptional() @IsEnum(SiteType) siteType?: SiteType;
  @IsOptional() @IsEnum(SiteStatus) status?: SiteStatus;
  @IsOptional() @IsUUID() parentSiteId?: string;
