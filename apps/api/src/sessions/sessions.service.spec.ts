...
describe('SessionsService', () => {
  ...
  beforeEach(() => {
    ...
    trafficEnforcement.supportsDisconnect.mockReset();
    trafficEnforcement.disconnectClient.mockReset();
    trafficEnforcement.supportsDisconnect.mockReturnValue(true);
    service = new SessionsService(db, audit, secureCredentials, networkCommands, trafficEnforcement);
  });
  ...
  it('expires grants and disconnects active sessions through the verified network control fabric', async () => {
    clientQuery
      .mockResolvedValueOnce({ rows: [], rowCount: 0 })
      ...
      .mockResolvedValueOnce({ rows: [], rowCount: 0 });
    query
      .mockResolvedValueOnce({ rows: [{ id: 'session-a', customerId: 'customer-a', routerId: 'router-a', username: 'alice', ipAddress: '10.0.0.10', macAddress: 'AA:BB:CC:DD:EE:FF', apiEndpoint: 'https://10.0.0.1', managementProtocol: 'MIKROTIK_REST', managementEnabled: true, credentialsEncrypted: 'enc' }], rowCount: 1 })
      .mockResolvedValueOnce({ rows: [{ hasActiveGrant: false }], rowCount: 1 });
    networkCommands.queue.mockResolvedValueOnce({ id: 'command-a', reused: false });
    trafficEnforcement.disconnectClient.mockResolvedValueOnce({ applied: true, verified: true, commandId: 'command-a', details: { removed: [{ service: 'hotspot', id: '*1' }] } });
    ...
    expect(trafficEnforcement.disconnectClient).toHaveBeenCalledWith(expect.objectContaining({ apiEndpoint: 'https://10.0.0.1', targetAddress: '10.0.0.10', username: 'alice' }), { username: 'admin', password: 'secret' }, 'MIKROTIK_REST', 'tenant-a', 'access-reconcile:session-a');
  });
  ...
  it('does not terminate a session when a renewal creates a new active grant during reconciliation', async () => {
    ...
    networkCommands.queue.mockResolvedValueOnce({ id: 'command-a', reused: false });
    trafficEnforcement.disconnectClient.mockResolvedValueOnce({ applied: true, verified: true, commandId: 'command-a', details: { removed: [] } });
    secureCredentials.decrypt.mockReturnValueOnce({ username: 'admin', password: 'secret' });
  ...
  it('fails closed and marks the session stale when verified disconnect is unavailable', async () => {
    clientQuery
      .mockResolvedValueOnce({ rows: [], rowCount: 0 })
      .mockResolvedValueOnce({ rows: [{ id: 'grant-expired', customerId: 'customer-a', routerId: 'router-a' }], rowCount: 1 })
      .mockResolvedValueOnce({ rows: [], rowCount: 1 })
      .mockResolvedValueOnce({ rows: [], rowCount: 0 });
    query.mockResolvedValueOnce({ rows: [{ id: 'session-a', customerId: 'customer-a', routerId: 'router-a', username: 'alice', ipAddress: '10.0.0.10', macAddress: null, apiEndpoint: 'https://10.0.0.1', managementProtocol: 'MIKROTIK_REST', managementEnabled: true, credentialsEncrypted: 'enc' }], rowCount: 1 });
    networkCommands.queue.mockResolvedValueOnce({ id: 'command-a', reused: false });
    trafficEnforcement.supportsDisconnect.mockReturnValueOnce(false);
    clientQuery
      .mockResolvedValueOnce({ rows: [], rowCount: 0 })
      .mockResolvedValueOnce({ rows: [], rowCount: 1 })
      .mockResolvedValueOnce({ rows: [], rowCount: 0 });
    ...
    expect(result.sessions[0]).toMatchObject({ sessionId: 'session-a', commandId: 'command-a', state: 'STALE', reason: 'NETWORK_ENFORCEMENT_UNAVAILABLE' });
    expect(networkCommands.markFailed).toHaveBeenCalledWith('tenant-a', ['command-a'], expect.any(Error));
    expect(trafficEnforcement.disconnectClient).not.toHaveBeenCalled();
  });
});
