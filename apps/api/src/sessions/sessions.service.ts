...
import { MikrotikRestAdapter } from '../routers/mikrotik-rest.adapter';
import { SecureNetworkCredentials, NetworkCredentials } from '../common/secure-network-credentials';
...
export class SessionsService {
  constructor(@Inject(PG_POOL) private readonly db: Pool, private readonly audit: AuditService, private readonly mikrotik: MikrotikRestAdapter, private readonly secureCredentials: SecureNetworkCredentials, private readonly networkCommands: NetworkCommandService) {}

  async list(tenantId: string, status?: SessionStatus, limit = 100) {
    const safeLimit = Math.min(Math.max(Math.trunc(limit || 100), 1), 500);
    const result = await this.db.query(`SELECT id, customer_id AS "customerId", router_id AS "routerId", username, ip_address AS "ipAddress", mac_address::text AS "macAddress", started_at AS "startedAt", ended_at AS "endedAt", bytes_in AS "bytesIn", bytes_out AS "bytesOut", bytes_total AS "bytesTotal", status, created_at AS "createdAt" FROM sessions WHERE tenant_id=$1 AND ($2::text IS NULL OR status=$2) ORDER BY started_at DESC LIMIT $3`, [tenantId, status ?? null, safeLimit]);
    return { data: result.rows, count: result.rowCount ?? 0 };
  }

  async get(tenantId: string, id: string) {
    const result = await this.db.query(`SELECT id, customer_id AS "customerId", router_id AS "routerId", username, ip_address AS "ipAddress", mac_address::text AS "macAddress", started_at AS "startedAt", ended_at AS "endedAt", bytes_in AS "bytesIn", bytes_out AS "bytesOut", bytes_total AS "bytesTotal", status, created_at AS "createdAt" FROM sessions WHERE tenant_id=$1 AND id=$2`, [tenantId, id]);
  ...
  private async resetRouterActiveUsers(client: PoolClient, tenantId: string, routerIds: string[]) {
    ...
    await client.query(`UPDATE routers r SET active_users=(SELECT count(*)::int FROM sessions s WHERE s.tenant_id=$1 AND s.router_id=r.id AND s.status='ACTIVE'), updated_at=now() WHERE r.tenant_id=$1 AND r.id=ANY($2::uuid[])`, [tenantId, uniqueRouterIds]);
  }
  ...
  async start(tenantId: string, input: StartSessionDto, auditContext: AuditContext = {}) {
    ...
    try {
      ...
      try {
        const result = await client.query(`INSERT INTO sessions (tenant_id, customer_id, router_id, username, ip_address, mac_address) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id, customer_id AS "customerId", router_id AS "routerId", username, ip_address AS "ipAddress", mac_address::text AS "macAddress", started_at AS "startedAt", status, bytes_in AS "bytesIn", bytes_out AS "bytesOut", bytes_total AS "bytesTotal"`, [tenantId, input.customerId ?? null, input.routerId ?? null, input.username?.trim() || null, input.ipAddress ?? null, input.macAddress ?? null]);
  ...
  async updateUsage(tenantId: string, id: string, input: UpdateSessionUsageDto) {
    const result = await this.db.query(`UPDATE sessions SET bytes_in=COALESCE($3,bytes_in), bytes_out=COALESCE($4,bytes_out) WHERE tenant_id=$1 AND id=$2 AND status='ACTIVE' RETURNING id, bytes_in AS "bytesIn", bytes_out AS "bytesOut", bytes_total AS "bytesTotal", status`, [tenantId, id, input.bytesIn ?? null, input.bytesOut ?? null]);
  ...
  async reconcileAccessState(tenantId: string, auditContext: AuditContext = {}) {
    ...
    const sessions = await this.db.query(`SELECT s.id, s.customer_id AS "customerId", s.router_id AS "routerId", s.username, s.ip_address AS "ipAddress", s.mac_address::text AS "macAddress", r.api_endpoint AS "apiEndpoint", r.management_protocol AS "managementProtocol", r.management_enabled AS "managementEnabled", r.management_credentials_encrypted AS "credentialsEncrypted" FROM sessions s LEFT JOIN routers r ON r.tenant_id=s.tenant_id AND r.id=s.router_id WHERE s.tenant_id=$1 AND s.status='ACTIVE' AND (NOT EXISTS (SELECT 1 FROM access_grants g WHERE g.tenant_id=s.tenant_id AND g.customer_id=s.customer_id AND g.status='ACTIVE' AND g.starts_at<=now() AND (g.ends_at IS NULL OR g.ends_at>now()) AND (g.router_id IS NULL OR g.router_id=s.router_id)) OR (EXISTS (SELECT 1 FROM customer_access_bindings b WHERE b.tenant_id=s.tenant_id AND b.customer_id=s.customer_id AND (b.router_id IS NULL OR b.router_id=s.router_id)) AND NOT EXISTS (SELECT 1 FROM customer_access_bindings b WHERE b.tenant_id=s.tenant_id AND b.customer_id=s.customer_id AND b.state='ACTIVE' AND (b.router_id IS NULL OR b.router_id=s.router_id) AND (b.expires_at IS NULL OR b.expires_at>now()))) OR (EXISTS (SELECT 1 FROM customer_service_state css WHERE css.tenant_id=s.tenant_id AND css.customer_id=s.customer_id) AND NOT EXISTS (SELECT 1 FROM customer_service_state css WHERE css.tenant_id=s.tenant_id AND css.customer_id=s.customer_id AND css.state IN ('ACTIVE','GRACE') AND css.effective_at<=now() AND (css.expires_at IS NULL OR css.expires_at>now()))))`, [tenantId]);

    const results: Array<Record<string, unknown>> = [];
    for (const session of sessions.rows) {
      const command = await this.networkCommands.queue(tenantId, { routerId: session.routerId ?? undefined, commandType: 'DISCONNECT_SESSION', actor: 'access-reconciliation', provider: session.managementProtocol ?? undefined, correlationId: `access-reconcile:${session.id}`, target: { sessionId: session.id, customerId: session.customerId, ipAddress: session.ipAddress, macAddress: session.macAddress, username: session.username }, request: { reason: 'ACCESS_GRANT_BINDING_OR_SERVICE_STATE_INVALID' } });
      if (!session.routerId || session.managementProtocol !== 'MIKROTIK_REST' || !session.managementEnabled || !session.apiEndpoint || !session.credentialsEncrypted) {
        await this.networkCommands.markFailed(tenantId, [command.id], new Error('Network disconnect could not be attempted: router management is unavailable or unsupported'));
      ...
      try {
        ...
        const enforcement = await this.mikrotik.disconnectClient(session.apiEndpoint, credentials, { ipAddress: session.ipAddress ?? undefined, username: session.username ?? undefined, macAddress: session.macAddress ?? undefined });
        ...
        if (!accessValid) {
          ...
          await this.networkCommands.markVerified(tenantId, command.id, { ...enforcement, entitlementVerified: false, action: 'SKIPPED_DUE_TO_RENEWAL' });
          results.push({ sessionId: session.id, commandId: command.id, state: 'RETAINED', reason: 'ACCESS_RENEWED_DURING_RECONCILIATION' });
        }
      } catch (error: unknown) {
        await this.networkCommands.markFailed(tenantId, [command.id], error);
        await this.markSessionStale(tenantId, session.id, session.routerId);
        results.push({ sessionId: session.id, commandId: command.id, state: 'STALE', reason: 'NETWORK_DISCONNECT_FAILED' });
      }
    }
  ...
  private async verifyAccessStillValid(tenantId: string, sessionId: string) {
    const result = await this.db.query(`SELECT (EXISTS (SELECT 1 FROM sessions s JOIN access_grants g ON g.tenant_id=s.tenant_id AND g.customer_id=s.customer_id WHERE s.tenant_id=$1 AND s.id=$2 AND g.status='ACTIVE' AND g.starts_at<=now() AND (g.ends_at IS NULL OR g.ends_at>now()) AND (g.router_id IS NULL OR g.router_id=s.router_id)) AND (NOT EXISTS (SELECT 1 FROM customer_access_bindings b JOIN sessions s ON s.tenant_id=b.tenant_id AND s.customer_id=b.customer_id WHERE s.tenant_id=$1 AND s.id=$2 AND (b.router_id IS NULL OR b.router_id=s.router_id)) OR EXISTS (SELECT 1 FROM sessions s JOIN customer_access_bindings b ON b.tenant_id=s.tenant_id AND b.customer_id=s.customer_id WHERE s.tenant_id=$1 AND s.id=$2 AND b.state='ACTIVE' AND (b.router_id IS NULL OR b.router_id=s.router_id) AND (b.expires_at IS NULL OR b.expires_at>now()))) AND (NOT EXISTS (SELECT 1 FROM sessions s JOIN customer_service_state css ON css.tenant_id=s.tenant_id AND css.customer_id=s.customer_id WHERE s.tenant_id=$1 AND s.id=$2) OR EXISTS (SELECT 1 FROM sessions s JOIN customer_service_state css ON css.tenant_id=s.tenant_id AND css.customer_id=s.customer_id WHERE s.tenant_id=$1 AND s.id=$2 AND css.state IN ('ACTIVE','GRACE') AND css.effective_at<=now() AND (css.expires_at IS NULL OR css.expires_at>now()))) AS "hasValidAccess"`, [tenantId, sessionId]);
