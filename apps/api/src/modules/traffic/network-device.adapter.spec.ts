...
describe('NetworkDeviceAdapterRegistry', () => {
  ...
  beforeEach(() => { mikrotik.readHotspotActive.mockReset(); global.fetch = jest.fn() as any; });
  ...
  it('reads connected UniFi clients through the official site/client API shape', async () => {
    ...
    const registry = new NetworkDeviceAdapterRegistry(config, mikrotik);
    await expect(registry.readClients({ routerId: 'r2', protocol: 'UNIFI_NETWORK_API', controllerEndpoint: 'https://api.ui.com/v1' })).resolves.toEqual([{ username: 'bob', address: '10.0.0.3', macAddress: 'BB', bytesIn: '9007199254740992', bytesOut: '42' }]);
  ...
  it('reads Meraki clients and nested usage counters', async () => {
    (global.fetch as jest.Mock).mockResolvedValue({ ok: true, status: 200, json: async () => [{ description: 'phone', ip: '10.0.0.4', mac: 'CC', usage: { recv: 1000, sent: 2000 } }] });
    const registry = new NetworkDeviceAdapterRegistry(config, mikrotik);
  ...
  it('reads OpenWrt clients through ubus session login, iwinfo devices and assoclist', async () => {
    (global.fetch as jest.Mock)
      .mockResolvedValueOnce({ ok: true, status: 200, json: async () => ({ result: [0, { ubus_rpc_session: 'sid-123' }] }) })
      .mockResolvedValueOnce({ ok: true, status: 200, json: async () => ({ result: [0, { devices: ['wlan0'] }] }) })
      .mockResolvedValueOnce({ ok: true, status: 200, json: async () => ({ result: [0, { results: [{ mac: 'AA:BB:CC:DD:EE:FF', rx_bytes: '9007199254740992', tx_bytes: '42' }] }] }) });
    const registry = new NetworkDeviceAdapterRegistry(config, mikrotik);
    await expect(registry.readClients({ routerId: 'r4', protocol: 'OPENWRT_UBUS', endpoint: 'https://router.local', credentials: { username: 'root', password: 'secret' } })).resolves.toEqual([{ macAddress: 'AA:BB:CC:DD:EE:FF', bytesIn: '9007199254740992', bytesOut: '42' }]);
    expect((global.fetch as jest.Mock).mock.calls[0][1].body).toContain('root');
    expect((global.fetch as jest.Mock).mock.calls[1][1].headers.Accept).toBe('application/json');
  });
  ...
  it('normalizes generic controller JSON for supported non-native adapters', async () => {
    ...
    const registry = new NetworkDeviceAdapterRegistry(config, mikrotik);
    await expect(registry.readClients({ routerId: 'r5', protocol: 'GENERIC_HTTP', controllerEndpoint: 'https://controller.example/clients' })).resolves.toEqual([{ username: 'dave', address: '10.0.0.5', macAddress: 'DD', bytesIn: '9007199254740992', bytesOut: '42' }]);
  });
  ...
  it('fails closed for protocols without an implemented native telemetry transport', async () => {
    const registry = new NetworkDeviceAdapterRegistry(config, mikrotik);
    await expect(registry.readClients({ routerId: 'r7', protocol: 'SNMP', endpoint: 'https://router.local' })).rejects.toThrow('SNMP telemetry requires');
  });
});
