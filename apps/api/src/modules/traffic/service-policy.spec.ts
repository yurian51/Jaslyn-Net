...
describe('resolveServicePolicy', () => {
  ...
  it('enforces package speed and returns remaining quota', () => {
    expect(resolveServicePolicy({
      ...
      maxDownloadMbps: 50,
      maxUploadMbps: 10,
      remainingBytes: 7_500,
    });
  });

  it('fails closed after quota exhaustion', () => {
    expect(resolveServicePolicy({
      dataLimitBytes: 1_000,
      usedBytes: 1_000,
  ...
  it('fails closed before activation', () => {
    expect(resolveServicePolicy({
      ...
      usedBytes: 0,
      now,
    }).state).toBe('NO_ACTIVE_SERVICE');
  });

  it('allows uncapped speed when package speed is unspecified', () => {
    expect(resolveServicePolicy({ usedBytes: 0, now })).toEqual({
      state: 'ACTIVE',
      maxDownloadMbps: Number.POSITIVE_INFINITY,
      maxUploadMbps: Number.POSITIVE_INFINITY,
      remainingBytes: null,
    });
  });
});
