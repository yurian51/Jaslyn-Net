...
export class TrafficEnforcementService {
  ...
  async evaluateAndApply(routerId: string, policy: FairnessPolicy, activeUsers: EnforcementUser[],
    targets: Record<string, EnforcementTarget> = {}, uploadRatio = 0.5, credentials?: NetworkCredentials,
    protocol: NetworkManagementProtocol = 'MIKROTIK_REST', tenantId?: string, correlationId?: string): Promise<EnforcementResult> {
    const state = this.fairnessService.evaluate(policy, activeUsers);
    const serviceLimits = Object.fromEntries(activeUsers.map((user) => [`${user.customerId}:${user.sessionId ?? ''}`, {
    ...
    let executableCommands = commands; let commandIds: string[] = [];
    if (this.networkCommands && tenantId && commands.length) {
      ...
      executableCommands = executable.map((entry) => entry.command); commandIds = executable.map((entry) => entry.id);
    }
    if (!executableCommands.length) return { applied: false, commandCount: 0, commands: [], commandIds: [], verifiedCommandIds: [], verificationFailures: 0, mode: state.mode, utilizationPercent: Number(state.utilizationPercent.toFixed(3)) };
    try { await adapter.apply(executableCommands, credentials); }
    catch (error) { if (commandIds.length && tenantId) await this.networkCommands!.markFailed(tenantId, commandIds, error); throw error; }
    if (commandIds.length && tenantId) await this.networkCommands!.markExecuted(tenantId, commandIds, { protocol, commandCount: executableCommands.length });
    let verifiedCommandIds: string[] = []; let verificationFailures = 0;
    if (commandIds.length && tenantId && adapter.verify) {
      try {
        ...
        verifiedCommandIds = verifiedIndexes.flatMap((index) => {
    ...
    return { applied: true, commandCount: executableCommands.length, commands: executableCommands, commandIds, verifiedCommandIds, verificationFailures, mode: state.mode, utilizationPercent: Number(state.utilizationPercent.toFixed(3)) };
  }
