  ...
  IF pkg.id IS NOT NULL THEN
    NEW.network_policy := jsonb_build_object(
      ...
      'session', jsonb_build_object(
        ...
        'interimUpdateSeconds', LEAST(300, GREATEST(30, FLOOR(pkg.duration_seconds / 20.0)::int))
      ),
...
SET network_policy = jsonb_build_object(
  ...
  'session', jsonb_build_object('sessionTimeoutSeconds', k.duration_seconds, 'interimUpdateSeconds', LEAST(300, GREATEST(30, FLOOR(k.duration_seconds / 20.0)::int))),
